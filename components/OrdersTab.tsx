...
import { BarChart2, ClipboardEdit, Trash2 } from 'lucide-react';
...
const OrdersTab: React.FC<OrdersTabProps> = ({ 
  ...
  onStatusChange, onDeleteOrder, onViewDetails, onOpenManualOrder 
}) => {
  ...
  const chartData = useMemo(() => {
    ...
    const manualSales = inventoryProducts.flatMap(p => (p.salesHistory || []).filter(s => !s.id.startsWith('ORDER-')).map(s => ({ date: toLocalISO(s.date), total: (Number(s.quantity) || 0) * (Number(s.unitPrice) || 0) })));
