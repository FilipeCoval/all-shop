...
import { 
  ...
  ChevronDown, ChevronRight, Globe, FileText, Copy, DollarSign, Package, TrendingUp, AlertCircle, Users, Bot, Sparkles, Loader2, Layers, BellRing, ShoppingCart, ClipboardList
} from 'lucide-react';
              ...
              {groupedInventory.map(([groupId, items]) => {
                ...
                return (
                  <React.Fragment key={groupId}>
                    <tr className={`hover:bg-gray-50 transition-colors ${isExpanded ? 'bg-blue-50/30' : ''}`}>
                      ...
                      <td className="px-4 py-4 text-right">
                          <div className="flex justify-end gap-1">
                              ...
                              <button onClick={() => onEdit(mainItem)} className="flex items-center gap-1 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors"><Edit2 size={14} /> Editar</button>
                    ...
                    {isExpanded && (
                        <tr className="bg-gray-50/50 border-b border-gray-200"><td colSpan={6} className="px-4 py-4"><div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm ml-10"><table className="w-full text-xs"><thead className="bg-gray-100 text-gray-500 uppercase"><tr><th className="px-4 py-2 text-left">Lote / Variante</th><th className="px-4 py-2 text-left">Origem</th><th className="px-4 py-2 text-center">Stock</th><th className="px-4 py-2 text-right">Custo</th><th className="px-4 py-2 text-right">Cashback</th><th className="px-4 py-2 text-right">Ações</th></tr></thead><tbody className="divide-y divide-gray-100">{items.map(item => { const itemStock = Math.max(0, (item.quantityBought || 0) - (item.quantitySold || 0)); return (<tr key={item.id} className="hover:bg-blue-50/30"><td className="px-4 py-2"><div className="font-bold text-gray-700">{item.variant || 'Padrão'}</div><div className="text-[10px] text-gray-400">{new Date(item.purchaseDate).toLocaleDateString()}</div></td><td className="px-4 py-2 text-gray-600">{item.supplierName || '-'}</td><td className="px-4 py-2 text-center font-bold text-gray-800">{itemStock} / {item.quantityBought}</td><td className="px-4 py-2 text-right text-gray-600">{formatCurrency(item.purchasePrice)}</td><td className="px-4 py-2 text-right"><span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${item.cashbackStatus === 'RECEIVED' ? 'bg-green-100 text-green-700' : item.cashbackStatus === 'PENDING' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-500'}`}>{item.cashbackValue > 0 ? formatCurrency(item.cashbackValue) : '-'}</span></td><td className="px-4 py-2 text-right"><div className="flex justify-end gap-1"><button onClick={() => onEdit(item)} className="p-1 bg-gray-100 text-gray-600 rounded hover:bg-gray-200"><Edit2 size={14}/></button><button onClick={() => onDelete(item.id)} className="p-1 bg-red-50 text-red-500 rounded hover:bg-red-100"><Trash2 size={14}/></button></div></td></tr>); })}</tbody></table></div></td></tr>
                    )}
                  </React.Fragment>
                );
              })}
