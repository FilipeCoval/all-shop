  ...
  return (
    <div className="animate-fade-in">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
            ...
            <div onClick={onOpenOnlineDetails} className="p-4 rounded-xl border bg-white shadow-sm flex flex-col justify-between h-full cursor-pointer hover:border-green-300 transition-colors relative overflow-hidden">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 flex gap-4 text-xs font-medium text-gray-500">
                ...
                <span className="text-green-600">Stock: {products.filter(p => p.status !== 'SOLD').length}</span>
                <span className="w-px h-4 bg-gray-300"></span>
                <span className="text-red-600">Esgotados: {products.filter(p => p.status === 'SOLD').length}</span>
            </div>
            <div className="p-4 border-b border-gray-200 flex flex-col lg:flex-row justify-between items-center gap-4">
                <div className="flex gap-2 w-full lg:w-auto">
                    <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as any)} className="py-2 px-3 border border-gray-300 rounded-lg text-sm bg-white">
                        <option value="ALL">Todos os Estados</option>
                        <option value="IN_STOCK">Em Stock</option>
                        <option value="SOLD">Esgotado</option>
                    </select>
                    <select value={cashbackFilter} onChange={(e) => setCashbackFilter(e.target.value as any)} className="py-2 px-3 border border-gray-300 rounded-lg text-sm bg-white">
                ...
                <div className="flex gap-2 w-full lg:w-auto">
                    ...
                    <button onClick={() => onOpenScanner('search')} className="bg-gray-700 text-white px-3 py-2 rounded-lg hover:bg-gray-900 transition-colors" title="Escanear Código de Barras">
                        <Camera size={18} />
                    </button>
                                    ...
                                    <tr className={`hover:bg-gray-50 transition-colors ${isExpanded ? 'bg-blue-50/30' : ''}`}>
                                        ...
                                        <td className="px-4 py-4 text-right">
                                            <div className="flex justify-end gap-1">
                                                ...
                                                <button onClick={() => onEdit(mainItem)} className="flex items-center gap-1 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors">
                                                    <Edit2 size={14} /> Editar Loja
                                                </button>
                                                <button onClick={() => onCreateVariant(mainItem)} className="p-1.5 text-blue-500 hover:bg-blue-50 rounded">
                                                    <Layers size={16} />
                                                </button>
                                                <button onClick={() => onDeleteGroup(groupId, items)} className="p-1.5 text-red-500 hover:bg-red-50 rounded">
                                                    <Trash2 size={16} />
                                                </button>
                                            </div>
                                        </td>
                                                    ...
                                                    <table className="w-full text-xs">
                                                        ...
                                                        <tbody className="divide-y divide-gray-100">
                                                            {items.map(p => { 
                                                                ...
                                                                const cashbackValue = p.cashbackValue || 0; 
                                                                const finalProfit = salePrice - purchasePrice + cashbackValue; 
                                                                const hasLossBeforeCashback = salePrice < purchasePrice; 
                                                                const profitColor = finalProfit > 0 ? 'text-green-600' : finalProfit < 0 ? 'text-red-600' : 'text-gray-500'; 
                                                                    ...
                                                                    <tr key={p.id} className="hover:bg-blue-50 transition-colors">
                                                                        ...
                                                                        <td className="px-4 py-3">
                                                                            {p.supplierName ? (
                                                                                <div>
                                                                                    ...
                                                                                    {p.supplierOrderId && (
                                                                                        <div 
                                                                                            className="text-[10px] text-gray-500 flex items-center gap-1 bg-gray-100 px-1.5 py-0.5 rounded w-fit mt-1 group cursor-pointer hover:bg-gray-200 transition-colors" 
                                                                        ...
                                                                        <td className="px-4 py-3 text-center">
                                                                            ...
                                                                            <div className="w-20 bg-gray-200 rounded-full h-1.5 overflow-hidden mx-auto">
                                                                                <div 
                                                                                    className={`h-full rounded-full ${ (p.quantityBought || 0) > 0 && ((p.quantitySold || 0) / (p.quantityBought || 1)) >= 1 ? 'bg-gray-400' : 'bg-blue-500'}`} 
                                                                        ...
                                                                        <td className="px-4 py-3 text-right flex justify-end gap-1">
                                                                            ...
                                                                            <button onClick={() => onDelete(p.id)} className="text-red-400 hover:bg-red-50 p-1.5 rounded bg-white border border-red-200 shadow-sm" title="Apagar lote">
                                                                                <Trash2 size={14}/>
                                                                            </button>
                                                                        </td>
                                                                    </tr> 
