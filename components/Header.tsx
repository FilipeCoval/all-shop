        ...
        <div className="flex items-center justify-end gap-2 md:gap-4 flex-1">
            ...
            <div className="flex items-center gap-2">
                {user ? (
                    <div className="relative group hidden sm:block">
                        <button 
                            ...
                            className="flex items-center gap-2 px-3 py-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-gray-700 dark:text-gray-200 font-medium border border-transparent hover:border-gray-200 dark:hover:border-gray-700"
                        >
                            <UserIcon size={20} />
                            <span className="text-base max-w-[100px] truncate hidden lg:block">{getFirstName()}</span>
                        </button>
                        {/* Dropdown Menu */}
                        <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform translate-y-2 group-hover:translate-y-0 p-1 z-50">
                            <div className="p-3 border-b border-gray-100 dark:border-gray-700">
                                <LoyaltyProgressBar user={user} />
                            </div>
                            <a href="#account" onClick={handleNav('account')} className="block px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg text-sm text-gray-700 dark:text-gray-200 font-medium">Minha Conta</a>
                    ...
                    <button 
                        onClick={onOpenLogin}
                        className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-full text-gray-700 dark:text-gray-200 font-bold hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-base"
                    >
                        Entrar
                    </button>
                )}
                ...
                >
                    ...
                    {cartCount > 0 && (
                        <span className="absolute top-0.5 right-0.5 inline-flex items-center justify-center w-4 h-4 text-[10px] font-bold text-white bg-red-600 rounded-full border border-white dark:border-gray-900 animate-bounce-slow">
                            {cartCount}
                        </span>
                    )}
                </button>
