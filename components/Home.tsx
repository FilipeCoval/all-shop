          ...
          >
              {categoryVisuals.map((cat, idx) => (
                  <div key={idx} onClick={() => handleCategoryClick(cat.name)} className="flex flex-col items-center gap-3 min-w-[100px] md:min-w-[140px] group/item flex-shrink-0 cursor-pointer transition-transform active:scale-95">
                      <div className={`w-20 h-20 md:w-32 md:h-32 rounded-full p-1 border-4 transition-all duration-300 overflow-hidden relative shadow-md bg-white dark:bg-[#0f172a] ${selectedCategory === cat.name ? 'border-primary ring-4 ring-primary/20 scale-105 shadow-xl z-10' : 'border-white dark:border-slate-700 group-hover/item:border-primary group-hover/item:shadow-lg group-hover/item:scale-105'}`}>
                          <img src={cat.image} alt={cat.name} draggable={false} className="w-full h-full object-cover rounded-full transition-transform duration-700 group-hover/item:scale-110 bg-gray-50 dark:bg-slate-800 select-none" />
                      </div>
                      <span className={`text-xs md:text-base font-bold transition-colors text-center leading-tight px-1 whitespace-nowrap ${selectedCategory === cat.name ? 'text-primary' : 'text-gray-600 dark:text-slate-300 group-hover/item:text-primary'}`}>{cat.name}</span>
                  </div>
              ))}
          </div>
      </section>
      ...
      <section className="py-4 bg-gray-50 dark:bg-[#020617] border-b border-gray-200/50 dark:border-slate-800 transition-colors duration-300">
          <div className="container mx-auto px-2 md:px-4 max-w-4xl">
              <div className="grid grid-cols-3 gap-2 md:gap-6">
                  <div className="flex flex-col md:flex-row items-center justify-center gap-1.5 md:gap-3 p-2 md:p-3 rounded-2xl transition-all duration-300 hover:bg-white dark:hover:bg-[#0f172a] hover:shadow-lg cursor-default group">
