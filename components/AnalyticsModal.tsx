...
const AnalyticsModal: React.FC<AnalyticsModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[70] p-4 animate-fade-in">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden transition-colors">
                <div className="p-6 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900 transition-colors">
                    <div>
                        <h3 className="font-bold text-xl text-gray-900 dark:text-white flex items-center gap-2">
                            <TrendingUp className="text-blue-600 dark:text-blue-400" /> Análise de Tráfego
                        </h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Monitorização de visitantes e origem geográfica.</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-gray-50 dark:bg-slate-950 space-y-6 transition-colors">
                    {/* Controls */}
                    <div className="flex justify-between items-center">
                        ...
                        <div className="text-right">
                            <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold">Total no Período</p>
                            <p className="text-2xl font-bold text-gray-900 dark:text-white">{totalVisitsPeriod}</p>
                        </div>
                    </div>

                    {/* Chart */}
                    <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm h-80 transition-colors">
                        {loading ? (
                    ...
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm transition-colors">
                        <h4 className="font-bold text-gray-800 dark:text-gray-200 mb-4 flex items-center gap-2">
                            <MapPin className="text-red-500 dark:text-red-400" size={20} /> Origem do Tráfego (Top 10)
                        </h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {locationStats.map(([location, count], idx) => (
                                <div key={location} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-slate-900 rounded-lg border border-gray-100 dark:border-slate-800 transition-colors">
                                    <div className="flex items-center gap-3">
                                        <span className="w-6 h-6 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center text-xs font-bold text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-slate-700">
                                            {idx + 1}
                                        </span>
                            ...
                            {locationStats.length === 0 && (
                                <p className="text-gray-400 dark:text-gray-500 italic col-span-2 text-center py-4">Sem dados de localização registados.</p>
                            )}
                        </div>
                    </div>
                </div>
