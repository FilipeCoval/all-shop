...
const ClientArea: React.FC<ClientAreaProps> = ({ user, orders, onLogout, onUpdateUser, wishlist, onToggleWishlist, onAddToCart, publicProducts }) => {
  ...
  const handleConfirmCancellation = async () => {
      ...
      try {
          const orderRef = db.collection('orders').doc(cancellationModalOrder.id);
          ...
          alert("Não foi possível cancelar a encomenda. Por favor, tente novamente ou contacte o suporte.");
      } finally {
                          ...
                          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                              ...
                              <div className="flex items-center gap-4">
                                  ...
                                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize
                                      ...
                                      orders[0].status === 'Pago' ? 'bg-cyan-100 text-cyan-800' :
                      ...
                      {(orders || []).map((order) => (
                          ...
                          <td className="px-6 py-4">
                              ...
                              ${order.status === 'Entregue' ? 'bg-green-100 text-green-800' : 
                                  ...
                                  order.status === 'Pago' ? 'bg-cyan-100 text-cyan-800' :
                          ...
                          <td className="px-6 py-4 text-right flex justify-end gap-2">
                              ...
                              {['Processamento', 'Pago'].includes(order.status) && (
                                  <button 
                                      onClick={() => setCancellationModalOrder(order)}
                  ...
                  <div className="flex items-start justify-between mb-4">
                      <div>
                          <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                              <AlertTriangle className="text-red-500"/> Cancelar Encomenda
                          </h3>
                          <p className="text-sm text-gray-500 mt-1">Ref: {displayOrderId(cancellationModalOrder.id)}</p>
                      </div>
                      <button onClick={() => setCancellationModalOrder(null)} className="text-gray-400 hover:text-gray-600">
                          <X size={24}/>
                      </button>
                  </div>
                  <p className="text-sm text-gray-600 mb-4">
                      Tem a certeza que quer cancelar esta encomenda? Esta ação não pode ser desfeita.
                  </p>
                  <div>
                      ...
                      <textarea
                          ...
                          rows={3}
                          placeholder="Ex: Encomenda de teste, item errado, etc."
                      />
                  </div>
                  <div className="flex justify-end gap-3 mt-6">
                      <button onClick={() => setCancellationModalOrder(null)} className="px-4 py-2 border rounded-lg text-gray-700 font-medium hover:bg-gray-50">
                          Voltar
                      </button>
                      <button 
